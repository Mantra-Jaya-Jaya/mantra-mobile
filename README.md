...
_(Isi folder features bisa bertambah seiring berkembangnya aplikasi)._
...
> [!NOTE]
> Jika `BASE_URL` tidak diset, aplikasi akan memakai fallback dev URL dan halaman login akan menampilkan peringatan agar kamu menjalankan app dengan `--dart-define=BASE_URL=...`.
...
- **Generate Keystore (Jika belum punya):**
    ...
    **Windows (dijalankan dari folder `\frontend\`):**
    ...
    _(Ingat baik-baik password yang Anda masukkan saat proses ini)._
...
- **Buat File `key.properties`:**
    ...
    _(Ganti `PASSWORD_KEYSTORE_ANDA` dengan password yang Anda buat sebelumnya)._
